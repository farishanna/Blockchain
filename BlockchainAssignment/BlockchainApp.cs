...
namespace BlockchainAssignment
{
    public partial class BlockchainApp : Form
    {
        ...
        // Mining settings
        String mineSetting;
        // Address that miner would like to mine for
        ...
        private void NewBlock_Click(object sender, EventArgs e)
        {
            ...
            {
                UpdateText("There are currently no transactions in the transaction pool\n\n");
                return;
            }
            if (comboBox1.SelectedItem == null)
            {
                UpdateText("Please choose a mining setting\n\n");
                return;
            }
            else if (comboBox1.SelectedItem.ToString() == "Sender Address")
            {
                address = publicKey.Text;
            }
            else if (comboBox1.SelectedItem.ToString() == "Reciever Address")
            {
                address = reciever.Text;
            }
            else
            {
                address = String.Empty;
            }
            // Stopwatch for multithreading
         //   System.Diagnostics.Stopwatch sw = new System.Diagnostics.Stopwatch();
           // sw.Start();
            // Retrieve pending transactions to be added to the newly generated Block
            List<Transaction> transactions = blockchain.GetPendingTransactions(mineSetting, address);
           ...
           // sw.Stop();
            UpdateText(blockchain.ToString());
            //UpdateText(sw.ElapsedMilliseconds.ToString());
        }
        ...
        {
            // Gets which setting is chosen
            mineSetting = comboBox1.Text;
        }
    }
}